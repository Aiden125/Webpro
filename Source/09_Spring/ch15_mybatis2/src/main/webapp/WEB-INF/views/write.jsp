...
<meta charset="UTF-8">
	...
	<script>
		...
		function chk(){
			if(! frm.empno.value){ // 아무것도 입력안하고 중복확인 눌렀을 때
...
<body>
	...
	<form action="write.do" method="post" name="frm">
		<table>
			...
			<tr>
				<th>입사일</th>
				<td><input type="date" name="tempHiredate" value="<%=new Date(System.currentTimeMillis()) %>">
			</tr>
			<tr>
				<th>급여</th>
			...
			<tr>
				<th>부서번호</th>
				<td><input type="number" name="comm" value="0">
					<select name="deptno">
						<c:forEach items="${depe.deptno }">${dept.deptno }-${dept.dname }-${dept.loc }</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
